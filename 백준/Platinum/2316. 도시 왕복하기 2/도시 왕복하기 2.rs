...
fn solve(stdin: &str) {
    ...
    let (n, p) = next!(usize, usize);
    let mut flow_graph = FlowGraph::new(n * 2);

    for i in 1..=n {
        flow_graph.connect(i, i + n, 1);
    }

    for _ in 0..p {
        let (u, v) = next!(usize, usize);
        flow_graph.connect(u + n, v, 1);
        flow_graph.connect(v + n, u, 1);
    }

    println!("{}", flow_graph.max_flow(n + 1, 2));
}
